...
const inboxEmails = [
  { title: "Welcome to RepoHive Mail", from: "RepoHive Team", body: "Your secure mailbox is now ready. You can receive workspace updates, system notifications, and team messages." },
  { title: "OTP Verification Successful", from: "Security", body: "Your account verification was successful. This helps keep your mailbox protected." },
  { title: "Project Workspace Invitation", from: "Douglas Hill", body: "You have been added to a RepoHive workspace. Open your dashboard to view tasks, repositories, and updates." }
];
...
function loadMailbox() {
  ...
  if (userEmail) userEmail.textContent = localStorage.getItem("verified_user") || "Verified User";
  showInbox();
}
...
function renderEmails(emails) {
  ...
  list.innerHTML = "";
  if (emails.length === 0) {
    list.innerHTML = `<div class="mail-item"><strong>No emails found</strong><small>This folder is empty.</small></div>`;
    return;
  }
  emails.forEach((mail, index) => {
    ...
    item.innerHTML = `<strong>${mail.title || mail.subject}</strong><small>${mail.from ? "From: " + mail.from : "To: " + mail.to}</small>`;
    list.appendChild(item);
    if (index === 0) { item.classList.add("active"); openEmail(mail, item); }
  });
}
...
function openEmail(mail, element) {
  ...
  document.getElementById("previewMeta").textContent = mail.from ? `From: ${mail.from}` : `To: ${mail.to}`;
...
function filterMail() {
  ...
  renderEmails(emails.filter(mail => JSON.stringify(mail).toLowerCase().includes(keyword)));
}
...
function sendChat() {
  ...
  input.value = "";
  showTyping();
  setTimeout(() => { removeTyping(); appendMessage(generateBotReply(message), "bot"); }, 900);
}
...
function appendMessage(text, sender) {
  ...
  wrapper.innerHTML = `<div class="avatar">${sender === "user" ? "👤" : "🤖"}</div><div class="bubble">${text}</div>`;
...
function generateBotReply(message) {
  ...
  if (text.includes("email") || text.includes("summarize")) return "Your mailbox shows recent updates about OTP verification, workspace invitations, and RepoHive activity notifications.";
  if (text.includes("sent")) return "Your sent email history is available inside the mailbox page.";
  if (text.includes("compose")) return "Sure. Start with a clear subject, write a short message, and end with a polite closing.";
  if (text.includes("otp")) return "OTP verification helps protect user access by confirming ownership of a phone number or email address.";
  if (text.includes("task") || text.includes("project")) return "RepoHive helps you organize tasks, repositories, project updates, and team collaboration in one workspace.";
